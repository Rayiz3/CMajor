...
<img src="{{ 'assets/img/illustration/cm202/9_6.png' | absolute_url }}" alt="image6" class="post" />
 ...
 &ensp;이 구조가 특별한 점은, **공간이나 시간적 제약이 없다**는 것입니다. 자료 크기에 상관 없이 일정한 크기의 배열(해시 테이블)을 만들 수 있어 공간의 제약이 없으며, 우리가 원하는 데이터의 해시값을 알기만 하면 그 값을 인덱스로 하여 곧바로 접근할 수 있으니 시간이 제약이 없습니다.

 &ensp;하지만 한 가지 문제가 있다면, **서로 다른 값이 같은 해시값을 가질 수도 있다**는 것입니다. 이렇게 되면 같은 인덱스에 서로 다른 두 값이 들어가야 하는 상황이 발생하죠. **이를 어떻게 해결할 것인지**가 해시 테이블의 주요 논의점이 될 것입니다.
