...
$$dist(u) = min\lbrace dist(u), dist(z) + l(z, u) \rbrace$$
...
<img src="{{ 'assets/img/illustration/cm301/7_22.png' | absolute_url }}" alt="image22" class="post" />
 ...
 &ensp;첫 번째 선 $(s, u_1)$을 제외한 각 선들은 적어도 한 번 $dist(v) = \min \lbrace dist(v), dist(u) + l(u, v) \rbrace$를 통한 업데이트를 거쳤을 것입니다. $(u_1, u_2)$ 같은 경우 $v = u_2, u = u+1$인 셈입니다. 다만 어디서 $u_1$나 $u_2$가 튀어나올지 모르니까, **단순하게 모든 선에 대해서** $dist(v) = \min \lbrace dist(v), dist(u) + l(u, v) \rbrace$를 비교해보는 겁니다.
