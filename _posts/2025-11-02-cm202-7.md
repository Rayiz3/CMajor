...
<img src="{{ 'assets/img/illustration/cm202/7_4.png' | absolute_url }}" alt="image4" class="post" />
 ...
 &ensp;일반적인 트리를 이진 트리로 변환할 때는 **LCRS(Left-Child-Right-Sibling)** 표기법을 사용하면 됩니다. 형제들 중에서 맨 왼쪽에 있는 자식은 그대로 두고, 나머지 자식들을 모두 부모 대신 자기 왼쪽에 있는 자식과 연결시켜버리는 방법이에요.
...
height h 일 때 T에서 node의 개수 n :     h+1 <= n <= 2^(h+1) - 1
