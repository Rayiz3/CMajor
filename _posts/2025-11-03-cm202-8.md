---
title: '[CM202] 08. 우선순위 큐와 힙 (Priority Queue & Heap)'
description: ''
author: Doun
...
image:
  ...
  alt: '[CM202] 우선순위 큐와 힙 (Priority Queue & Heap)'
---
...
## 우선순위 큐(Priority queue)
 ...
 &ensp;**우선순위 큐**<sup>Priority Queue</sup>는 각 요소마다 우선수위가 부여되어 높은 우선순위의 요소를 우선적으로 제거할 수 있는 구조의 ADT입니다. 이름이 큐이긴 하지만, 사실상 큐와는 전혀 다른 정책을 가지고 있는 셈이죠.
...
## 힙 (Heap)
 ...
 &ensp;**힙**<sup>Heap</sup>은 이진 트리를 기반으로 하여, 모든 노드에 대해 **부모-자식 간 대소 관계가 일관된** 자료 구조입니다. 말이 조금 어려운데 풀어서 설명 하자면, 어떤 노드이던, **부모 노드가 자식노드보다 순서 상 우위**(클 수도, 작을 수도 있죠)에 있다는 것입니다.

<img src="{{ 'assets/img/illustration/cm202/8_3.png' | absolute_url }}" alt="image3" class="post" />

 &ensp;힙은 **완전 이진 트리**<sup>Complete Binary Tree</sup>의 형태로 표현할 수 있어야 합니다. 완전 이진 트리는 루트부터 맨 밑 층까지, 맨 밑 층에서는 왼쪽 끝에서부터 차례대로 노드가 전부 채워진 상태를 말합니다. 이 때문에 **이진 힙**<sup>Binary Heap</sup>이라고 부르기도 합니다.
...
## 힙 복원하기 (Reheapifying)
 ...
 &ensp;완벽히 정렬된 힙에 새로운 요소를 하나 추가하면 곧 정렬이 깨지게 됩니다(정말정말 우연히 정렬 조건을 만족하는 값이 들어가는게 아니라면요). 그러면 정렬 조건을 만족하게 만들어 다시 힙을 복구해야 합니다. 새로 들어온 값을 알맞은 자리로 옮겨서 말이죠. 이를 **힙 복구**<sup>Reaheapifying</sup>이라고 합니다.
...
## 우선순위 큐 써 먹기 - 힙 정렬 (Heap sort)
 ...
 &ensp;결국 우선순위 큐는 매번 우선순위에 따라 요소들을 정렬시킨 후, 맨 첫 번째 요소를 빼가는 원리였습니다. 그 말은, 이 큐 구조 자체를 정렬에 써 먹을 수도 있다는 뜻이죠! 이를 **힙 정렬**<sup>Heap Sort</sup>이라고 합니다.
