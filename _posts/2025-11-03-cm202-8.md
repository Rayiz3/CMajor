...
public interface MaxPriorityQueue<Key> {
	...
	Key max();          // 1순위 요소 반환
	void insert(Key k); // 요소 추가
	Key delMax();       // 요소 제거 (1순위 요소)
}
```
...
| less(Key v, Key w) | v : 요소1, w : 요소2 | boolean  | v가 w보다 작은지 확인     |
| exch(Key[] a, int i, int j) | a : 배열, i : 위치1, j : 위치2 | 배열 a에서 i번째와 j번째를 교환 |
| swim(Key k)        | k : 요소            | -        | 요소를 위로 이동시켜 정렬  |
| sink(Key k)        | k : 요소            | -        | 요소를 밑으로 이동시켜 정렬 |
...
``` java
private static void exch(Key[] a, int i, int j) {
	Key swap = a[i];  // i번째 요소 저장
