...
$$C(i, j) = \min_{i \leq k \leq j} \lbrace  C(i, k) + C(k+1, j) + m_{i-1} * m_k * m_j  \rbrace$$
...
<img src="{{ 'assets/img/illustration/cm301/10_6.png' | absolute_url }}" alt="image6" class="post" />
 ...
 &ensp;점 $i$에서 $j$까지, 점 $\lbrace 1, 2, …, k \rbrace$만 사용해서 도달할 수 있는 가장 짧은 경로의 길이를 $\text{dist(i, j, k)}$라고 합시다. 이 경로는 **점 $k$를 지나는 경우**와 **그렇지 않은 경우** 두 가지 중 하나에 해당할 것입니다.
...
<img src="{{ 'assets/img/illustration/cm301/10_8.png' | absolute_url }}" alt="image8" class="post" />
 ...
 &ensp;TSP도 동적 계획법으로 해결해보겠습니다. 점 $S=\lbrace  1, …, n  \rbrace$이 주어져 있을 때, 점 $1$에서 출발해 모든 점을 한 번씩 지나 $j \neq 1$에 도착하는 최단 경로의 길이를 $C(S, j)$라고 합시다.
...
- **경우 1.** $S = \lbrace 1 \rbrace$ 뿐이며 $j=1$ : 주어진 점이 1, 도착점도 1이므로 경로의 길이는 **0**입니다.

- **경우 2.** $\lbrace 1 \rbrace \subset S$ 이며 $j=1$ : $j \neq 1$라고 정했으므로, 길이는 모두 **무한**입니다.
...
$$C(\lbrace 1 \rbrace, 1) = 0, \; C(S, 1) = \infty$$

$$C(S, j) = \min_{i \in S, \; i \neq j} \lbrace  C(S - \lbrace  j  \rbrace, i) + d_{ij}  \rbrace$$
...
$$\min_j \lbrace  C(\lbrace  1, …, n  \rbrace, j) + d_{j1}  \rbrace$$
