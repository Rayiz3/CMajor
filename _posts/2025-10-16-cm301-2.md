...
$$
T_A(n) = \max_{|X| = n} \lbraceT_A(X)\rbrace
$$
...
$$
T_{\Pi}(n) = \min_{\Pi 를 풀 수 있는 A} \lbraceT_A(n)\rbrace = \min_{\Pi 를 풀 수 있는 A} \lbrace\max_{|X| = n} \lbraceT_A(X)\rbrace\rbrace
$$
...
$$
\log{n!} = \log{ \lbrace\;(\frac{n}{e})^n\sqrt{2\pi n}\;\rbrace } = n\log{(\frac{n}{e})} + \frac{1}{2}\log{2\pi n}
$$
