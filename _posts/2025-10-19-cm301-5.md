...
$$
T(n) = T(max \lbracean, bn\rbrace) + \Theta(n) \quad (an + bn = n-1)
$$

 &ensp;그렇다면 우리는 랜덤 퀵정렬 때와 같은 방법으로 평균 실행 시간을 계산해볼 수 있습니다.

$$
E[T(n)] = \sum_{k=0}^{n-1} \frac{1}{n} \; \left\lbrace E[T(max \lbracek, n-k-1\rbrace)] + \Theta(n) \right\rbrace
$$

$$
= \frac{1}{n} \; \sum_{k=0}^{n-1} E[T(max \lbracek, n-k-1\rbrace)] + \Theta(n)
$$
