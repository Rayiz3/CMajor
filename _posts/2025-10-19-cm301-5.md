...
<img src="{{ 'assets/img/illustration/cm301/5_6.png' | absolute_url }}" alt="image6" class="post" />
 ...
 &ensp;도망가지 말아요, 천천히 풀어봅시다. 우선 배열을 길이 5의 작은 배열들 m개로 나누라고 합니다.

<img src="{{ 'assets/img/illustration/cm301/5_7.png' | absolute_url }}" alt="image7" class="post" />

 &ensp;그리고 그 작은 배열들마다 3번째로 작은 값, 그러니까 중간 값을 찾습니다. m개의 중간값들이 나올 것입니다. 이들의 배열을 새로이 **y[1, ..., m]**라고 정의했네요.

<img src="{{ 'assets/img/illustration/cm301/5_8.png' | absolute_url }}" alt="image8" class="post" />

 &ensp;이제 y에서도 중간값을 찾습니다. 즉 중간값들 중에서도 **중간-중간값**을 찾는 것이죠.
...
<img src="{{ 'assets/img/illustration/cm301/5_10.png' | absolute_url }}" alt="image10" class="post" />
 ...
 &ensp;그리고 배열 y에 속한 값들은 길이가 5인 배열의 중간값입니다. 때문에 이들보다 작은 두 개의 값이 반드시 존재할 것입니다.

<img src="{{ 'assets/img/illustration/cm301/5_11.png' | absolute_url }}" alt="image11" class="post" />

 &ensp;그러므로 중간-중간값은 적어도 $\lfloor \frac{m}{2} \rfloor * 3$ 개의 값보다는 큰 값이 되는 것이죠. 그런데 m은 길이 n의 배열을 길이 5 단위로 분할했을 때 생기는 하위 배열들의 개수죠? 그렇기 때문에 $m = \lfloor \frac{n}{5} \rfloor$ 로 나타낼 수 있습니다.
 
 &ensp;따라서 중간-중간값은 항상 그 보다 작은 $\lfloor \lfloor \frac{n}{5} \rfloor \frac{1}{2} \rfloor * 3 \sim \frac{3}{10}n$ 개의 값을 가지게 됩니다. 다르게 말하면, 중간-중간값은 그보다 큰 최대 $\frac{7}{10}n$ 개의 값을 가질 수 있다는 뜻입니다. 그러니 최악의 경우, 이 알고리즘은 $\frac{7}{10}n$ 길이의 하위 배열로 재귀를 호출하게 됩니다.

 &ensp;이제 총 계산량을 종합해봅시다.
 
 - 각 중앙값들을 찾을 때는 모든 값들을 한 번씩 순회하게 됩니다. 때문에 **중앙값 m개를 찾는데 걸리는 시간**은 $O(n)$입니다.
 
 - **중간-중간값을 찾는데 필요한 시간**은 배열 y의 길이 만큼, 즉 $O(n/5)$입니다.
 
 - 마지막으로 최악의 경우 **재귀로 호출되는 하위배열의 크기**는 $\frac{7}{10}n$입니다.
...
$$
 ...
 &ensp;그러므로 중간-중간값 알고리즘의 시간 복잡도는 $O(n)$, 즉 선형 시간 안에 끝낼 수 있게 됩니다.