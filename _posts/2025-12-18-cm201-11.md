...
| 요소  | 의미                      | 예시                    |
| :--- | :-----------              | :--------------------- |
| $V$  | 기호들의 집합               | $\lbrace a, b, A, B, S \rbrace$  |
| $T$  | 단말 기호로 이루어진 하위집합 | $\lbrace a, b \rbrace$ |
| $S$  | 첫 시작 기호               | $S$ |
| $P$  | 생성 규칙                  | $\lbrace S \rightarrow Aba, A \rightarrow BB, B \rightarrow ab, AB \rightarrow b \rbrace$ |
...
$$S \rightarrow \lambda$$
 ...
 &ensp;CSG의 대표적인 예시는 $\lbrace a^nb^nc^n \rbrace$입니다. a, b, c가 각각 n번씩 연달아 있다는 뜻입니다. 이는 아래와 같은 생성 규칙으로 만들어낼 수 있습니다.
...
$$A \rightarrow w$$
 ...
 &ensp;CFG의 대표적인 예시는 $\lbrace a^nb^n \rbrace$입니다. 이에 대한 규칙은 간단하게 나타낼 수 있습니다.
...
- 입력 알파벳(input alphabet) $\Sigma \subseteq \Gamma - \lbrace b \rbrace$

- 초기 상태 $q_0 \in Q$

- 최종 상태 $F \subseteq Q$

- 전이 함수 $\delta : (Q - F) \times \Gamma \rightarrow Q \times \Gamma \times \lbrace L, R \rbrace$
...
<img src="{{ 'assets/img/illustration/cm201/11_17.png' | absolute_url }}" alt="image17" class="post" />
 ...
 &ensp;이제 제어장치가 테이프에 적혀진 알파벳을 하나씩 읽을 것입니다. 그리고 **현재 상태**($(Q - F)$)와 **읽은 기호**($\Gamma$)에 따라 1. **상태**를 바꾸거나($Q$) 2. **새로운 알파벳**을 쓰거나($\Gamma$) 3. 제어장치를 **왼쪽 또는 오른쪽**으로 이동할 수 있습니다($\lbrace L, R \rbrace$). 이 때 새로운 알파벳은 입력 알파벳에 속하지 않은 테이프 알파벳($\Gamma - \Sigma$)을 사용할 수도 있습니다.
...
### 튜링 머신 - 예시
 ...
 &ensp;좀 더 쉽게 와닿을 수 있도록, 이전에 CSG 언어의 예시로 들었던 $\lbrace a^nb^nc^n \rbrace$ 로 튜링 머신을 만들어봅시다. 기본적으로 이렇게 설계할 거에요.
