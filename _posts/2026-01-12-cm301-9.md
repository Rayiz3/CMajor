...
<img src="{{ 'assets/img/illustration/cm301/9_1.png' | absolute_url }}" alt="image1" class="post" />
 ...
 &ensp;이번에는 탐욕적 알고리즘만큼 빠르지는 않지만, 확실하게 답을 찾을 수 있는 디자인 패러다임인 **동적 계획법**<sup>Dynamic Programming</sup>과 이를 활용한 문제들에 대해 살펴보겠습니다.
...
<img src="{{ 'assets/img/illustration/cm301/9_3.png' | absolute_url }}" alt="image3" class="post" />
 ...
 &ensp;그래서 동적 계획법에서는 **작은 문제들을 먼저 계산한 후, 이를 기억**합니다. $f(3)$부터 미리 계산 해 놓으면 $f(4)=f(3)+f(2)$에서도, $f(5)=f(4)+f(3)$에서도, $f(3)$이 나올 때 마다 함수를 또 호출할 필요 없이 미리 계산된 값을 넣기만 하면 되니까요. 그리고 이렇게 계산한 $f(4)$, $f(5)$들로 값을 기억해 놓으면 또 써먹을 수 있습니다. 이렇게 값을 기억하는 방식을 **메모이제이션**<sup>Memoization</sup>이라고 합니다.
...
## 가장 긴 수열
 ...
 &ensp;주어진 수열 $X=\rbrace a_1, ..., a_n\rbrace $이 있을 때, 그 안에서 **가장 길이가 긴 증가 수열의 길이**를 계산해봅시다. 여기서 증가 수열은 $X$에서 나열된 순서가 변하지 않아야 합니다.

<img src="{{ 'assets/img/illustration/cm301/9_5.png' | absolute_url }}" alt="image5" class="post" />

 &ensp;우리는 이 수열을 **방향이 있는 비순환 그래프**<sup>DAG</sup>로 나타낼 수 있습니다. 각 수가 점이고, 수가 증가하는 방향으로 선이 연결되는 방식으로요.
...
$$L(i) = 1 + \max_{(i, j) \in E} \rbrace  L(j) \rbrace $$
...
$$K(w) = \max_{w_i \leq w}\rbrace  K(w-w_i) + v_i \rbrace $$
...
$$C(j) = \min_{d}\rbrace  C(j-d_i) + 1 \rbrace $$
