...
## 가장 긴 수열
 ...
 &ensp;주어진 수열 $X=\lbracea_1, ..., a_n\rbrace$이 있을 때, 그 안에서 **가장 길이가 긴 증가 수열의 길이**를 계산해봅시다. 여기서 증가 수열은 $X$에서 나열된 순서가 변하지 않아야 합니다.
...
$$L(i) = 1 + \max_{(i, j) \in E} \lbrace L(j) \rbrace$$
...
$$K(w) = \max_{w_i \leq w}\lbrace K(w-w_i) + v_i \rbrace$$
...
$$C(j) = \min_{d}\lbrace C(j-d_i) + 1 \rbrace$$
